...
export default function CategoryManager() {
  const { categories, addCategory, updateCategory, removeCategory } = usePlannerStore()
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-surface-100 p-5">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-surface-800">🏷️ 카테고리</h3>
        <button
      ...
      <div className="space-y-2 mb-3">
        {categories.map((cat) => (
          ...
          >
            ...
            <span className="flex-1 text-sm font-medium text-surface-700">{cat.name}</span>
            <div
      ...
      {(adding || editingId) && (
        <div className="border-t border-surface-100 pt-3 space-y-3">
          ...
          <div className="flex gap-2">
            ...
            {editingId && (
              <button
                onClick={() => { removeCategory(editingId); setEditingId(null); setName('') }}
